...
class GameState:
    ...
    def start_round(self):
        ...
        for p in self.players:
            ...
            p.current_shop = roll_shop(p, self.pool, self.unit_db)
    ...
    def action_budget(self) -> int:
        if self.stage == 1: return 5
        elif self.stage == 2: return 10
        elif self.stage == 3: return 15
        elif self.stage == 4: return 20
        return 25
        
    def resolve_round(self) -> float:
        # Matchmaking: for simplicity, we just random pair non-eliminated players
        ...
        for i in range(0, len(active)-1, 2):
            matches.append((active[i], active[i+1]))
        
        if len(active) % 2 == 1:
            ghost = random.choice([p for p in active if p != active[-1]])
            matches.append((active[-1], ghost))
        ...
        for p1, p2 in matches:
            ...
            if winner == "A":
                p1.win_streak += 1
                p1.loss_streak = 0
                if not p2.is_agent or (p2.is_agent and matches[-1] != (p1, p2)):
                    p2.health -= damage
                    p2.win_streak = 0
                    p2.loss_streak += 1
                if p1 == self.agent: agent_won = True
                if p2 == self.agent: agent_lost = True
            elif winner == "B":
                p2.win_streak += 1
                p2.loss_streak = 0
                p1.health -= damage
                p1.win_streak = 0
                p1.loss_streak += 1
                if p2 == self.agent: agent_won = True
                if p1 == self.agent: agent_lost = True
        ...
        # Reward
        reward = 0.0
        if agent_won: reward += 0.2
        if agent_lost: reward -= 0.1
    ...
    def apply_action(self, action: int):
        ...
        if action == ACTION_BUY_XP:
            ...
            reroll(p, self.pool, self.unit_db)
    ...
    def action_reward(self, action: int) -> float:
        # Small dense rewards could be added here
        return 0.0
    ...
    def to_observation(self) -> np.ndarray:
        ...
        for t in self.all_traits:
            if t in active:
                obs.extend([1.0, 1.0]) # active_count_normalized, breakpoint_progress
            else:
                obs.extend([0.0, 0.0])
                
        # Opponents (7 * 34)
        for op in self.players:
            if op == self.agent: continue
            if op.is_eliminated:
                obs.extend([0.0] * 34)
            else:
                op_vec = [
