...
class Unit:
    ...
    def scale_stats(self):
        ...
        self.ability_damage *= multiplier
        # Armor/MR/AS typically don't scale with star level in TFT, just HP and Damage.
...
class UnitDatabase:
    ...
    def create_unit(self, unit_id, star_level=1, is_frontline=True):
        if unit_id not in self.unit_data:
            return None
        data = copy.deepcopy(self.unit_data[unit_id])
        unit = Unit(**data, star_level=star_level, is_frontline=is_frontline)
        unit.scale_stats()
        return unit
...
def remove_copies(player, unit_id, star_level, count=3):
    removed = 0
    # Prefer removing from bench first, then board
...
def try_combine(player, unit_id, star_level, unit_db: UnitDatabase):
    """Checks and combines 3 identical units into a higher star level."""
    if star_level >= 3:
        return
    ...
    if copies >= 3:
        ...
        if placed:
            # Recursively check for the next combine level
